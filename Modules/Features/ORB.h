 ...
 *
 * ORB feature descriptor using the implementation found in ORB-SLAM, ORB-SLAM2 and ORB-SLAM3
 */
...
class ORB : public Descriptor{
public:
    ORB(int nScales, float fScaleFactor) : nScales_(nScales), fScaleFactor_(fScaleFactor) {
    ...
    void describe(const cv::Mat& im, std::vector<cv::KeyPoint>& vKeys, cv::Mat& desc);
...
private:
    ...
    std::vector<float> vScaleFactor_, vInvScaleFactor_;
    std::vector<cv::Mat> vImagePyramid_;
